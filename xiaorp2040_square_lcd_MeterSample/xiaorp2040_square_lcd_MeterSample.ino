...
class LGFX_XIAORP2040_SPI_ST7789 : public lgfx::LGFX_Device
{
    lgfx::Panel_ST7789      _panel_instance;
    lgfx::Bus_SPI       _bus_instance;   // SPIバスのインスタンス
//    lgfx::Light_PWM     _light_instance;
  public:
    LGFX_XIAORP2040_SPI_ST7789(void)
    {
      { // バス制御の設定を行います。
        ...
        cfg.pin_dc    = D0;  //I26    // SPIのD/Cピン番号を設定  (-1 = disable)
      ...
      /*
            { // バックライト制御の設定を行います。（必要なければ削除）
              ...
              cfg.pin_bl = D2; //IO28         // バックライトが接続されているピン番号
...
LGFX_XIAORP2040_SPI_ST7789 lcd;
...
#define BUZZER_PIN D3              // ブザーのピン番号
#define BEAT 500                  // 音の長さ（0.5秒間隔
#define MOT_PIN D9 
...
void setup(void)
{
  ...
  Serial.begin(115200);
  tone(BUZZER_PIN, 262,BEAT); // ド
  delay(BEAT);
  tone(BUZZER_PIN, 294,BEAT); // レ
  delay(BEAT);
  tone(BUZZER_PIN, 330,BEAT); // ミ
  delay(BEAT);
